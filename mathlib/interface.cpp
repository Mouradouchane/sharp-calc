...
static void print_parsed_expression(node& expression_node , size_t tab = 1) {
	...
	if( expression_node.right != nullptr ) print_parsed_expression(*expression_node.right, tab + 3);

	for (size_t i = 1; i <= tab; i++) {
		std::cout << '\t';
	}
	std::cout << expression_node.value << '\n';

	if ( expression_node.left  != nullptr ) print_parsed_expression(*expression_node.left , tab + 3);
